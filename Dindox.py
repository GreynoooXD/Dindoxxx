...
def login():
	try:
		...
		try:
			basariheker = requests.get('https://graph.facebook.com/oauth?fields=id&access_token='+tokenku[0], cookies={'cookie':cok})
...
def dump_massal():
	...
	for userr in uid:
		try:
			col = ses.get('https://graph.facebook.com/v3.1/'+userr+'?fields=friends.limit(5000)&access_token='+tokenku[0], cookies = {'cookies':cok}).json()
